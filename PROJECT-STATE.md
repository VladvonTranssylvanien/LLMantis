...
| 14 | **DNS rebinding defeats the SSRF guard — hypothesis, code path confirmed.** `netguard.assert_public_host` resolves the hostname (`netguard.py:58`) and returns; the connection is then made by an independent second resolution (`art50check.py:125`, `scanner.py:69`). A name the attacker controls with TTL 0 — public on the first lookup, `127.0.0.1` on the second — would pass the guard. The two-resolution path was read; **no rebinding nameserver was stood up, so the race itself is unproven.** Not reachable today: both callers sit behind the site's Basic-Auth. The redirect half of this class is handled correctly (`art50check.py:119` sets `follow_redirects=False` and re-runs the guard on every hop). **Fix before the password comes off**, since that is the moment `/api/art50check` becomes anonymous and internet-facing | before removing Basic-Auth | Bogdan |
