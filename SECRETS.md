...
| LLMantis API keys (`llm_live_...`) | Issued via `POST /api/keys`, hash stored in Postgres | ✅ shown to the caller once, never recoverable | Whoever calls the endpoint |
...
- Any caller can currently mint a key for any `org_id` — there is no check
  that they own that organization. This is the authentication gap tracked in
  `PROJECT-STATE.md` (technical debt #9). Do not treat these keys as a
  security boundary between organizations until that is fixed.
